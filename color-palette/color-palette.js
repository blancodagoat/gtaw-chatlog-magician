(function ($) {
  ...
  function init() {
    ...
    $colorPalette
      ...
      .on('click.colorPalette', '.color-item', applyColorToSelection);
    $output
      .off('click.colorPalette')
      .on('click.colorPalette', '.colorable', handleTextElementClick);
    $output
      .off('mousedown.colorPalette')
      .on('mousedown.colorPalette', '.colorable', handleDragStart);
    $output.off('mouseup.colorPalette').on('mouseup.colorPalette', '.colorable', handleDragEnd);
    $output
      .off('mouseover.colorPalette')
      .on('mouseover.colorPalette', '.colorable', handleDragOver);
    ...
    $output
      .off('click.clearOutside.colorPalette')
      .on('click.clearOutside.colorPalette', function (e) {
    ...
    $output.off('mouseleave.colorPalette').on('mouseleave.colorPalette', function (_e) {
      if (!coloringMode) return;
    ...
    $output
      .off('selectstart.colorPalette', '.colorable')
      .on('selectstart.colorPalette', '.colorable', function (e) {
        if (coloringMode) {
          e.preventDefault();
          return false;
        }
  ...
  function showToast(message) {
    ...
    $('body').append(toast);
    setTimeout(() => {
      toast.fadeOut(CONFIG.TOAST_FADEOUT_MS, function () {
        $(this).remove();
      });
  ...
  function setupClosePaletteHandler() {
    $(document)
      .off('click.closePalette')
      .on('click.closePalette', function (e) {
        if (!coloringMode) return;
  ...
  function _getElementsBetween(_startEl, _endEl) {
    const allSpans = $output.find('span.colorable').toArray();

    const startIndex = allSpans.indexOf(_startEl);
    const endIndex = allSpans.indexOf(_endEl);
