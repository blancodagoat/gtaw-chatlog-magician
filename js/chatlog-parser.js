$(document).ready(function () {
    ...
    // --- Begin full coloring mode logic ---
    ...
    function processOutput() {
        ...
        // Get actual output container width and font
        const outputElem = $output[0];
        const maxPixelWidth = outputElem.clientWidth;
        const computedStyle = window.getComputedStyle(outputElem);
        // Compose font shorthand string for canvas context and CSS font property
        ...
        chatLines.forEach((line) => {
            ...
            // Use legacy character-based wrapping for normal mode
 ...
 */
// Legacy, character-based line breaking for normal mode
...
// Pixel-based wrapping for canvas/export mode
function addPixelLineBreaksAndHandleSpans(text, maxPixelWidth, font, outputElem) {
    // Setup a hidden measurement span
    let measureSpan = document.getElementById('measureSpanForPixelWrap');
    if (!measureSpan) {
        measureSpan = document.createElement('span');
        measureSpan.id = 'measureSpanForPixelWrap';
    ...
    if (font) measureSpan.style.font = font;
    if (outputElem) {
        const computed = window.getComputedStyle(outputElem);
        ...
        measureSpan.style.wordSpacing = computed.wordSpacing;
    ...
    for (let i = 0; i < text.length; i++) {
        if (text[i] === '<' && text.substr(i, 5) === '<span') {
            let spanEnd = text.indexOf('>', i);
            currentSpan = text.substring(i, spanEnd + 1);
            let match = currentSpan.match(/class="([^"]+)"/);
            spanClass = match ? match[1] : '';
            i = spanEnd;
            inSpan = true;
            result += currentSpan;
            currentLine = '';
            ...
            currentLine += text[i];
            measureSpan.innerHTML = inSpan ? `<span class="${spanClass}">${currentLine}</span>` : currentLine;
            if (measureSpan.offsetWidth > maxPixelWidth && currentLine.length > 1) {
                // Remove current char from line, add break, start new line
                let lastChar = currentLine.slice(-1);
                let lineToAdd = currentLine.slice(0, -1);
                if (inSpan) {
                    result += `${lineToAdd}</span><br><span class="${spanClass}">${lastChar}`;
                } else {
                    result += `${lineToAdd}<br>${lastChar}`;
                }
...
// --- End of addLineBreaksAndHandleSpans ---
