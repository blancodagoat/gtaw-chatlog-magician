(function () {
  ...
  const _UPDATE_KEY = 'jStorage_update';
  ...
  let storageSize = 0;
  let _updateTimeout = false;
  let _lastUpdate = 0;
  let pubSubHandlers = {};
  let _lastPubSubTime = Date.now();
  let ttlTimeout;
  ...
  const processPubSub = () => {
    ...
    validMessages.forEach(([_timestamp, channel, data]) => {
    ...
    _lastPubSubTime = Math.max(...validMessages.map(([timestamp]) => timestamp), now);
  };
