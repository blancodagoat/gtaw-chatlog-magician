(function (global, factory) {
  ...
  const pickerTemplate = (() => {
    const ieDivs = isIE
      ? Array.from({ length: 6 }, (_, i) => `<div class='sp-${i + 1}'></div>`).join('')
      : '';
  ...
  function _createPalette(colors, selectedColor, className, options) {
  ...
  function mergeOptions(options, _element) {
  ...
  class ColorPicker {
    ...
    initialize() {
      ...
      const _isColorInput =
        isInput && $element.attr('type') === 'color' && this.supportsColorInput();
    ...
    handleDocumentClick(e) {
      ...
      if (!$target.closest(this.$container).length && !$target.closest(this.$replacer).length) {
    ...
    togglePalette() {
      ...
      $button.text(
        isHidden ? this.options.togglePaletteMoreText : this.options.togglePaletteLessText
      );
    }
